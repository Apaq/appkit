![Built With Stencil](https://img.shields.io/badge/-Built%20With%20Stencil-16161d.svg?logo=data%3Aimage%2Fsvg%2Bxml%3Bbase64%%2BCjxzdHlsZSB0eXBlPSJ0ZXh0L2NzcyI%2BCgkuc3Qwe2ZpbGw6I0ZGRkZGRjt9Cjwvc3R5bGU%%2BCg%3D%3D&colorA=16161d&style=flat-square)

# WebstoreJS

This is a platform for adding profound app support in your web application including an app store if you like. Webstore is inspired by how apps, widgets etc. are build and interregates in iOS and Android. It lets you build and use apps, widgets and extensions in your web application and delivers a unified way of exposing data between those.

This opens up for the possibility of sharing UI components with others via a centralized app store as they will all build on the same foundation.

## TODO

* Isolation
* App bundle dependencies
* Move core to seperate library